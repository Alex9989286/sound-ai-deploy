...
async def lifespan(app: FastAPI):
    ...
    # 加载 ONNX 模型
    SESSION = ort.InferenceSession("model_7classes.onnx")
    print("✅ ONNX model loaded!")
    
    # 加载类别
    CLASSES = np.load("classes_7classes.npy", allow_pickle=True)
    print(f"✅ Classes loaded: {len(CLASSES)} classes")
    print(f"   Classes: {list(CLASSES)}")
...
# 特征提取
# ============================================
def extract_features(file_path, sr=SR, n_mfcc=N_MFCC, max_len=MAX_LEN, n_mels=N_MELS):
    try:
        y, sr = librosa.load(file_path, sr=sr, duration=5.0)
...
def sync_process_audio(file_path: str):
    try:
        features = extract_features(file_path)
        if features is None:
            return {"error": "Feature extraction failed", "label": "unknown", "confidence": 0.0}
        
        # ONNX 推理
        input_name = SESSION.get_inputs()[0].name
        predictions = SESSION.run(None, {input_name: features})[0][0]
        ...
        top3 = [(CLASSES[i], float(predictions[i])) for i in top3_idx]
